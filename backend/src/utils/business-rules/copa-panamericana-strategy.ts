...
export class CopaPanamericanaStrategy implements BusinessRulesStrategy {
  ...
  async validateChoreographyCreation(
    ...
    // Copa Panamericana allows maximum 4 choreographies per country per category
    const maxAllowed = this.getMaxChoreographiesPerCountryPerCategory();
    if (existingChoreographiesCount >= maxAllowed) {
      throw new BadRequestException(
        `Copa Panamericana allows maximum ${maxAllowed} choreographies per country per category. ${request.country} already has ${existingChoreographiesCount} in ${request.category} category.`
      );
    }
